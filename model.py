...
class Car(mesa.Agent):
    ...
    def strat_select_route(self) -> None:
        ...
        # 2)
            ...
            if Gl.DELTA_T < abs(self.expected_travel_R14 - self.expected_travel_R23):
                self.selected_route = self.model.possible_routes()[
                    np.argmin([self.expected_travel_R14, self.expected_travel_R23])]
    ...
    def strat_maybe_change_route_4link(self) -> None:
        if self.selected_route == Gl.R14:
            if self.expected_travel_R14 < self.expected_travel_R23:
                if self.waited_at_junction > (
                        self.expected_travel_R23 - self.expected_travel_R14) * Gl.J2_WAIT_THRESHOLD:
            ...
            if self.expected_travel_R23 < self.expected_travel_R14:
                if self.waited_at_junction > (
                        self.expected_travel_R14 - self.expected_travel_R23) * Gl.J2_WAIT_THRESHOLD:
    ...
    def strat_maybe_change_route_J1(self) -> None:  # 2 functions depending on which junction the agent is
        if self.selected_route == Gl.R14 or self.selected_route == Gl.R153:
            if self.selected_route == Gl.R14:
                if self.expected_travel_R14 < self.expected_travel_R23:
                    if self.waited_at_junction > (
                            self.expected_travel_R23 - self.expected_travel_R14) * Gl.J1_WAIT_THRESHOLD:
                        self.selected_route = Gl.R23
                        self.waited_at_junction = 0
                else:
                    self.selected_route = Gl.R23
                ...
                if self.expected_travel_R153 < self.expected_travel_R23:
                    if self.waited_at_junction > (
                            self.expected_travel_R23 - self.expected_travel_R153) * Gl.J1_WAIT_THRESHOLD:
                        self.selected_route = Gl.R23
                        self.waited_at_junction = 0
                else:
                    self.selected_route = Gl.R23
            ...
            assert (self.selected_route == Gl.R23)
            if (self.expected_travel_R23 < self.expected_travel_R14) and (
                    self.expected_travel_R23 < self.expected_travel_R153):
                if self.expected_travel_R14 < self.expected_travel_R153:
                    if self.waited_at_junction > (
                            self.expected_travel_R14 - self.expected_travel_R23) * Gl.J1_WAIT_THRESHOLD:
                        self.selected_route = Gl.R14
                        self.waited_at_junction = 0
                else:
                    assert (self.expected_travel_R153 <= self.expected_travel_R14)
                    if self.waited_at_junction > (
                            self.expected_travel_R153 - self.expected_travel_R23) * Gl.J1_WAIT_THRESHOLD:
    ...
    def strat_maybe_change_route_J2(self) -> None:
        if self.selected_route == Gl.R14:
            if self.expected_travel_R14 < self.expected_travel_R153:
                if self.waited_at_junction > (
                        self.expected_travel_R153 - self.expected_travel_R14) * Gl.J2_WAIT_THRESHOLD:
                    self.selected_route = Gl.R153
                    self.waited_at_junction = 0
            else:
                self.selected_route = Gl.R153
            ...
            if self.expected_travel_R153 < self.expected_travel_R14:
                if self.waited_at_junction > (
                        self.expected_travel_R14 - self.expected_travel_R153) * Gl.J2_WAIT_THRESHOLD:
                    self.selected_route = Gl.R14
                    self.waited_at_junction = 0
            else:
                self.selected_route = Gl.R14
...
class Model_5link_with_bus(Model_5link):
    ...
    def __init__(self, n_car, n_bus, verbose=False):
        ...
        self.infrastructure[Gl.E_SPAWN_BUS] = Tasep(self.n_bus, Gl.E_SPAWN_BUS, pos_start=(0, 0), pos_end=(0, 0),
